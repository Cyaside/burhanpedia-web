    ...
    <header className="sticky top-0 z-40 border-b bg-white">
      ...
      <div className="page-container grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-3 py-3 md:grid-cols-[190px_minmax(0,1fr)_auto] md:gap-x-6">
        <Link
          ...
          aria-label="Beranda Burhanpedia"
        >
          <Image
        ...
        <form
          ...
          role="search"
          className="order-3 col-span-2 flex min-w-0 md:order-none md:col-span-1"
        >
...
function CategoryNavigation({
  ...
  return (
    <CategoryLinks
      ...
      pathname={pathname}
      activeCategoryId={searchParams.get("categoryId")}
    />
  );
}
...
function CategoryLinks({
  ...
  return (
    <>
      <Link
        href="/products"
        className={`inline-flex min-h-11 items-center hover:text-primary ${pathname === "/products" && !activeCategoryId ? "text-primary" : "text-muted-foreground"}`}
      >
        Semua produk
      </Link>
      {categories
        ...
        .map((category) => (
          <Link
            ...
            className={`inline-flex min-h-11 items-center hover:text-primary ${pathname === "/products" && activeCategoryId === category.id ? "text-primary" : "text-muted-foreground"}`}
          >
            {category.name}
          </Link>
        ))}
      <Link
        href="/stores"
        className={`inline-flex min-h-11 items-center hover:text-primary ${pathname.startsWith("/stores") ? "text-primary" : "text-muted-foreground"}`}
      >
        Jelajahi toko
      </Link>
    </>
