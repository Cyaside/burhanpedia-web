...
export default function ProductDetailPage() {
  ...
  const addToCart = useMutation({
    mutationFn: (variantId: string) => commerceApi.addItem(variantId),
  ...
  return (
    <div className="min-h-dvh bg-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-24 pt-6 sm:px-6">
        ...
        {product.isPending && (
          <p className="mt-8 text-sm text-slate-600">Memuat produk…</p>
        )}
        {product.isError && (
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-2">
                ...
                <div className="mt-3 flex gap-2">
                  {data.images.map((image, index) => (
                    <button
                      ...
                      className={`relative size-16 overflow-hidden rounded-md border ${selectedImage === index ? "border-blue-700" : "border-slate-300"}`}
                    >
                      <Image
                        src={image.url}
            ...
            <div className="space-y-5 rounded-xl border bg-white p-5">
              <p className="text-sm text-slate-600">
                {data.category?.name ?? "Produk"} · {data.store.name}
              </p>
              ...
              <p className="text-sm text-slate-600">
                {variant
                  ? `Stok ${variant.availableQuantity}`
                  : `Total stok ${data.availableQuantity}`}
              </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {data.variants.map((item) => (
                      <button
                        ...
                        className={`rounded-md border px-3 py-2 text-sm disabled:opacity-50 ${selectedVariant === item.id ? "border-blue-700 bg-blue-50" : "border-slate-300"}`}
                      >
              ...
              <div className="border-t pt-4 text-sm leading-relaxed text-slate-700">
              ...
              <Button
                ...
                onClick={async () => {
                  if (!variant || !(await ensureAuthenticated(router))) return;
                  addToCart.mutate(variant.id);
                }}
              >
